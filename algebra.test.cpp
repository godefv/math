...
using add_operation_t=algebra::add_operation_t<decltype(geometric_group_3d), one_t, mult_t, inverse_t, double>;

template<class A, class B> constexpr auto operator+(A const& a, B const& b){
	return group::apply_operation<add_operation_t::operation_t>(a,b);
}
std::ostream& operator<<(std::ostream& out, group::generated_element_t<add_operation_t::operation_t, auto, auto> const& addition){
	//return algebra::print<decltype(geometric_group_3d), one_t, mult_t, inverse_t, double>(addition);
