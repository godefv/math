...
namespace godefv::math{
	...
	struct bilinear_operation_t{
		...
		//operation with zero
		static auto constexpr apply(Zero auto, auto const&){return zero;}
		static auto constexpr apply(auto const&, Zero auto){return zero;}
		...
		static auto constexpr apply(group::generated_power_t<add_operation_t, Scalar auto,auto> const& a, auto const& b){
		...
		static auto constexpr apply(A const& a, group::generated_power_t<add_operation_t, Scalar auto,auto> const& b){
	...
	auto constexpr inverse(Number auto const& a){return 1./a;}
	...
	auto constexpr inverse(NonSimpleScalarExpression auto const& a){
	...
	auto constexpr operator*(A const& a, B const& b){
   	...
   	template<Expression A, class B> requires ((NonSimpleScalar<A> || NonSimpleScalar<B>) && requires(B b){inverse(b);})
	auto constexpr operator/(A const& a, B const& b){
		return a*inverse(b);
	}
	...
	namespace detail{
		...
		std::ostream& format_multiplication_operand(std::ostream& out, group::Generated<mult_operation_t> auto const& operand){
			return out<<operand;
		}
	}
