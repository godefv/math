...
namespace godefv::math{
	// (xy)'=x'y+xy'
	auto constexpr differentiate(Symbol variable, group::generated_by_operation_t<mult_operation_t, auto, auto> const& ab){
		return ab.first*differentiate(variable, ab.second)+differentiate(variable, ab.first)*ab.second;
	}

	// (x^y)'=yx'(x^(y-1)) if x,x',y commute
	Scalar{ScalarOperandT}
	auto constexpr differentiate(Symbol variable, group::generated_power_t<mult_operation_t, Scalar, ScalarOperandT> const& operand){
		using group::power;
